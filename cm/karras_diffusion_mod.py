...
class KarrasDenoiser:
    ...
    def consistency_losses(
        ...
        x_noisy = micro['X'] # noisy image
        condition = micro['condition'] # other coils
        if model_kwargs is None:
            model_kwargs = {}
        # if noise is None:
        #     noise = th.randn_like(x_noisy)
        ...
        ### Denoising training
        with th.no_grad() and autocast('cuda'):
            x_0_bar = noise_model(condition).detach()

        res = x_noisy - x_0_bar
        [b, c, h, w] = x_0_bar.shape

        sigma_est = th.std(res, dim = (1,2,3), keepdim = True)
        noise_est = res / sigma_est
        noise_mean = th.mean(noise_est, dim = (1,2,3), keepdim = True)
        noise = noise_est - noise_mean.detach()

        x_start = x_0_bar + noise_mean.detach() * (sigma_est)

        noise = noise.view(b, c, -1)
        rand_idx = th.randperm(noise.shape[-1])
        noise = noise[:, :, rand_idx].view(b, c, h, w).detach()
        ### End
        ...
        if self.loss_norm == "l1":
            ...
            diffs_J = th.abs(distiller - x_noisy)
            loss = mean_flat(diffs + diffs_J) * weights

        elif self.loss_norm == "l2":
            # existing L2 parts
            diffs   = (distiller - distiller_target) ** 2    # consistency
            diffs_J = (distiller - x_noisy) ** 2             # data tether
            l2_term = mean_flat(diffs + diffs_J) * weights   # (B,)
        
            # --- NEW: KL term between N(distiller, t^2 I) || N(distiller_target, t^2 I)
            # KL = 0.5 * ||mu_p - mu_q||^2 / t^2     (constant drops out)
            inv_var = 1.0 / append_dims(t**2 + 1e-8, dims)   # (B,1,1,1)
            kl_per_pix = 0.5 * diffs * inv_var               # same shape as diffs
            kl_term = mean_flat(kl_per_pix) * weights        # (B,)
        
            # combine
            lam = getattr(self, "kl_lambda", 0.0)            # or use the kl_lambda arg
            loss = l2_term + lam * kl_term
        
        elif self.loss_norm == "l2_kl":
            # ---------------- L2 PART ----------------
            diffs   = (distiller - distiller_target) ** 2
            diffs_J = (distiller - x_noisy) ** 2
            l2_term = mean_flat(diffs + diffs_J) * weights
        
            # ---------------- KL PART ----------------
            eps = 1e-8
            k   = 4.0
            M   = 512
            B, C, H, W = distiller.shape
            dims = len(x_noisy.shape) - 1
        
            # Student residuals
            res_pred = (distiller - x_noisy)
        
            # Reference Gaussian
            t_b = append_dims(t, dims)                  
            mu_ref    = torch.zeros(B, device=distiller.device)  
            sigma_ref = (t_b.view(B) + 1e-12)                     
        
            kl_loss = torch.zeros(B, device=distiller.device)
        
            for i in range(B):
                sr_samples = res_pred[i].reshape(-1, 1).detach()
        
                left  = (mu_ref[i] - k * sigma_ref[i]).item()
                right = (mu_ref[i] + k * sigma_ref[i]).item()
                if right - left < 1e-6:
                    left  = (mu_ref[i] - 1e-3).item()
                    right = (mu_ref[i] + 1e-3).item()
                intensities = torch.linspace(left, right, M, device=distiller.device).view(-1, 1)
                dx = (right - left) / (M - 1 + 1e-12)
        
                # KDE for student residual distribution
                kde_sr = KernelDensity(bandwidth=0.05, kernel='gaussian')
                kde_sr.fit(sr_samples)
                p_pdf = kde_sr.score_samples(intensities).exp().view(-1)
        
                # Reference Gaussian pdf
                mu_i  = mu_ref[i]
                sig_i = sigma_ref[i].clamp_min(1e-12)
                z = intensities.view(-1)
                q_pdf = torch.exp(-0.5 * ((z - mu_i) / sig_i) ** 2) / (sig_i * math.sqrt(2.0 * math.pi))
        
                # Convert pdfs to pmfs and normalize
                p = (p_pdf * dx).clamp_min(eps); p = p / (p.sum() + eps)
                q = (q_pdf * dx).clamp_min(eps); q = q / (q.sum() + eps)
        
                # One-way KL: KL(student || reference)
                kl_div = torch.sum(p * (torch.log(p) - torch.log(q)))
                kl_loss[i] = kl_div
        
            # ---------------- COMBINE ----------------
            loss_per_sample = l2_term + kl_lambda * kl_loss
            ...
            if x_start.shape[-1] < 256:
                ...
                x_noisy = F.interpolate(x_noisy, size=224, mode="bilinear")
            loss = (
                self.lpips_loss((distiller + 1) / 2.0, (distiller_target + 1) / 2.0)
                +
                0.1 * self.lpips_loss((distiller + 1) / 2.0, (x_noisy + 1) / 2.0)
                ) * weights
        else:
