...
class NoiseModelTrainLoop():
    def __init__(
        ...
        # --- resume main model if a checkpoint was provided ---
        self.resume_step = 0
        self._load_and_sync_model_parameters()
        # if we resumed, align counters so saving/logging continue from the file's step
        if self.resume_step:
    ...
    def _load_and_sync_model_parameters(self):
        # On your codepath, find_resume_checkpoint() returns None; we fall back to CLI arg.
        resume_checkpoint = find_resume_checkpoint() or self.resume_checkpoint
        if resume_checkpoint:
            # e.g., "./save/.../model110000.pt" -> 110000
            self.resume_step = parse_resume_step_from_filename(resume_checkpoint)
            if dist.get_rank() == 0:
                logger.log(f"loading model from checkpoint: {resume_checkpoint}...")
            state = dist_util.load_state_dict(resume_checkpoint, map_location=dist_util.dev())
            self.model.load_state_dict(state)
        # keep replicas (if any) consistent
        dist_util.sync_params(self.model.parameters())
        dist_util.sync_params(self.model.buffers())
    ...
    def _load_and_sync_target_parameters(self):
        ...
        if resume_checkpoint:
            ...
            resume_target_checkpoint = resume_checkpoint
    ...
    def val_dwi(self):
        ...
        with torch.no_grad():
            for i, v in enumerate(self.val_data):
                X = v['X'].float().to(dist_util.dev())
                H, W = X.shape[-2 : ]
                condition = v['condition'].float().to(dist_util.dev())
                pred = self.model(condition)
                noisy = ((X + 1) / 2).clamp(0, 1).permute(0,1,3,2)
                denoised = ((pred + 1) / 2).clamp(0, 1).permute(0,1,3,2)

                transforms.ToPILImage()(noisy[0]).convert('L').save('{}/{}_{}_noisy.png'.format(val_dir_now, self.global_step, i))
                transforms.ToPILImage()(denoised[0]).convert('L').save('{}/{}_{}_denoised.png'.format(val_dir_now, self.global_step, i))

                res_raw = (noisy[0] - denoised[0]).squeeze(0).float().cpu()
                vmax = res_raw.abs().max()
                vmax = torch.clamp(vmax, min=1e-8)
                res_signed = 0.5 + 0.5 * (res_raw / vmax)
                transforms.ToPILImage()(res_signed.detach().to('cpu', dtype=torch.float32)).convert('L').save('{}/{}_{}_residual.png'.format(val_dir_now, self.global_step, i))
        self.model.train()
    ...
    def val(self):
        ...
        with torch.no_grad():
            for i, v in enumerate(self.val_data):
                ...
                condition = v['condition'].float().to(dist_util.dev())
                pred = self.model(condition)
                noisy = ((X + 1) / 2).clamp(0, 1)
                denoised = ((pred + 1) / 2).clamp(0, 1)

                if i % 4 == 0:
                    noisy_combined = torch.zeros((1, 1, H, 4 * W + 30)).to(dist_util.dev())
                    denoised_combined = torch.zeros((1, 1, H, 4 * W + 30)).to(dist_util.dev())
                noisy_combined[:, :, :, (W + 10)* (coil_id) : (W + 10) * coil_id + W] = noisy
                denoised_combined[:, :, :, (W + 10)* (coil_id) : (W + 10) * coil_id + W] = denoised

                if (i + 1) % 4 == 0:
                    transforms.ToPILImage()(noisy_combined[0]).convert('L').save('{}/{}_{}_noisy.png'.format(val_dir_now, self.global_step, i // 4 + 1))
                    transforms.ToPILImage()(denoised_combined[0]).convert('L').save('{}/{}_{}_denoised.png'.format(val_dir_now, self.global_step, i // 4 + 1))
                if i >= save_slice * 4:
                    break
        self.model.train()
    ...
    def forward_backward(self, batch):
        self.mp_trainer.zero_grad()
        for i in range(0, len(batch['X']), self.microbatch):
            micro = {}
            micro['X'] = batch['X'][i : i + self.microbatch].to(dist_util.dev())
            micro['condition'] = batch['condition'][i : i + self.microbatch].to(dist_util.dev())

            last_batch = (i + self.microbatch) >= batch['X'].shape[0]

            X, condition = micro['X'].float(), micro['condition'].float()
            pred = self.model(condition)
            if last_batch or not self.use_ddp:
                losses = self.l1loss(pred, X)
            else:
                with self.ddp_model.no_sync():
                    losses = self.l1loss(pred, X)
            ...
            if self.global_step % self.log_interval == 0:
                logger.log(datetime.now())
                logger.log(f"Step: {self.step}, Loss: {losses:.10f}...")
...
class CMTrainLoop(TrainLoop):
    ...
    def val_dwi(self):
        ...
        with torch.no_grad() and autocast('cuda'):
            for i, v in enumerate(self.val_data):
                ...
                # Stage I prediction
                x_0_bar = self.noise_model(condition).detach()
                res = X - x_0_bar
                sigma_est = th.std(res, dim = (1,2,3), keepdim = True)
                noise_est = res / sigma_est
                noise_mean = th.mean(noise_est, dim = (1,2,3), keepdim = True)

                # Stage II prediction
                pred = self.diffusion.denoise(self.target_model, X, sigma_est.view(X.shape[0]))[1] - noise_mean.detach() * (sigma_est)

                x_0_bar = ((x_0_bar + 1) / 2).clamp(0, 1).permute(0,1,3,2)
                noisy = ((X + 1) / 2).clamp(0, 1).permute(0,1,3,2)
                denoised = ((pred + 1) / 2).clamp(0, 1).permute(0,1,3,2)

                transforms.ToPILImage()(noisy[0]).convert('L').save('{}/{}_{}_noisy.png'.format(val_dir_now, self.global_step, i))
                transforms.ToPILImage()(x_0_bar[0]).convert('L').save('{}/{}_{}_regressed.png'.format(val_dir_now, self.global_step, i))
                transforms.ToPILImage()(denoised[0]).convert('L').save('{}/{}_{}_denoised.png'.format(val_dir_now, self.global_step, i))
    ...
    def forward_backward(self, batch):
        dev = dist_util.dev()
        need_rmt = getattr(self.diffusion, "loss_norm", None) in {"l2_rmt", "l1_rmt", "l1_rmt_no_j", "lpips_rmt"}
    
        self.mp_trainer.zero_grad()
        for i in range(0, len(batch['X']), self.microbatch):
            micro = {}
            micro['X']         = batch['X'][i : i + self.microbatch].float().to(dev)
            micro['condition'] = batch['condition'][i : i + self.microbatch].float().to(dev)

            # --- Thread X_rmt into the microbatch when needed ---
            if need_rmt:
                if 'X_rmt' not in batch:
                    raise RuntimeError(
                        "RMT loss selected (loss_norm = {}), but 'X_rmt' is missing from the batch. "
                        "Make sure MRIDataset returns X_rmt and you passed --rmt_npy_path."
                        .format(self.diffusion.loss_norm)
                    )
                micro['X_rmt'] = batch['X_rmt'][i : i + self.microbatch].to(dev).to(micro['X'].dtype)
        
        # self.mp_trainer.zero_grad()
        # for i in range(0, len(batch['X']), self.microbatch):
        #     micro = {}
        #     micro['X'] = batch['X'][i : i + self.microbatch].float().to(dist_util.dev())
        #     micro['condition'] = batch['condition'][i : i + self.microbatch].float().to(dist_util.dev())
