...
class KarrasDenoiser:
    def __init__(
        ...
        if loss_norm.startswith("lpips"):
            self.lpips_loss = LPIPS(replace_pooling=True, reduction="none")
        self.rho = rho # shapes how noise levels σ are spaced across the training grid
    ...
    def consistency_losses(
        self,
        model,      # the student UNet used for training
        micro,      # mini-batch dict from dataloader
        num_scales, # number of discrete noise levels in σ-schedule
        model_kwargs=None,
        noise_model=None,
        target_model=None,
        teacher_model=None,
        ...
        if self.loss_norm == "l1":
            ...
            loss = mean_flat(diffs + diffs_J) * weights # hybrid L2 + J-style regularization
        ...
        elif self.loss_norm == "lpips_rmt":
            x_rmt = micro.get('X_rmt', None)
            if x_rmt is None:
                raise ValueError("X_rmt is missing in micro dictionary")
            else:
                x_rmt = x_rmt.to(dtype=x_noisy.dtype, device=x_noisy.device)
            ...
            if x_start.shape[-1] < 256:
                ...
                x_rmt = F.interpolate(x_rmt, size=224, mode="bilinear")
            
            diffs = self.lpips_loss((distiller + 1) / 2.0, (distiller_target + 1) / 2.0)
            diffs_rmt = 0.1 * self.lpips_loss((distiller + 1) / 2.0, (x_rmt + 1) / 2.0)

            loss = (diffs + diffs_rmt) * weights
        else:
            raise ValueError(f"Unknown loss norm {self.loss_norm}")

        terms = {}
        terms["loss"] = loss
        terms["diffs"] = diffs.mean()
        
        if self.loss_norm in ("lpips_rmt"):
            terms["diffs_rmt"] = diffs_rmt.mean()
